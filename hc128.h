...
void DecryptMessage(HC128_State *state, uint8 *ciphertext, uint8 *plaintext, uint64 msglength) {
    uint64 i;
    uint32 j;

    /* Decrypt the message, each time 4 bytes are decrypted */
    for (i = 0; (i + 4) <= msglength; i += 4, ciphertext += 4, plaintext += 4) {
        /* Generate 32-bit keystream and store it in state->keystreamword */
        OneStep(state);
        /* Decrypt 32 bits of the ciphertext */
        ((uint32 *)plaintext)[0] = ((uint32 *)ciphertext)[0] ^ state->keystreamword;
    }
    /* Decrypt the last message block if the message length is not a multiple of 4 bytes */
    if ((msglength & 3) != 0) {
        OneStep(state);
        for (j = 0; j < (msglength & 3); j++) {
            *(plaintext + j) = *(ciphertext + j) ^ ((uint8 *)&state->keystreamword)[j];
        }
    }
}

/* Decrypt a message using HC-128 */
void HC128_Decrypt(uint8 *key, uint8 *iv, uint8 *ciphertext, uint8 *plaintext, uint64 msglength) {
    HC128_State state;
    ...
    /* Decrypt the message */
    DecryptMessage(&state, ciphertext, plaintext, msglength);
}